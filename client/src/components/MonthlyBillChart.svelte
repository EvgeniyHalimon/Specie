    ...
    $: reactiveShowSort = (input: any): any => {
      if (input === 1) {
        return (sortedData = data.sort(
          (a: any, b: any) => a[x].charCodeAt(0) - b[x].charCodeAt(0)
        ));
      }
      if (input === 2) {
        return (sortedData = data.sort((a: any, b: any) => a[y] - b[y]));
      }
      if (input === 3) {
        return (sortedData = data.sort((a: any, b: any) => b[y] - a[y]));
      }
    };
    $: reactiveXVals = sortedData.map((el: any) => el[x]);
    $: reactiveYVals = sortedData.map((el: any) => el[y]);
      ...
      <g class="y-axis" transform="translate({marginLeft}, 0)">
        {#each reactiveYTicks as tick, i}
          <g class="tick" opacity="1" transform="translate(0, {reactiveYScale(tick)})">
            ...
            <text x={-marginLeft} y="10">{yFormat === "%" ? Number(reactiveYTicksFormatted[i]) * 100 + yFormat : reactiveYTicksFormatted[i] + yFormat}</text>
          </g>
        {/each}
